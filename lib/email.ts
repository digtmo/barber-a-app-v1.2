...
export interface BarberReservationAlertData {
  barberEmail: string;
  barberName: string;
  barberSlug: string;
  clientName: string;
...
function buildBarberAlertHtml(data: BarberReservationAlertData): string {
  const domain = process.env.NEXT_PUBLIC_BARBER_DOMAIN ?? 'tubarber.com';
  const agendaUrl = `https://${data.barberSlug}.${domain}/acceso`;
  const dateFormatted = formatDateEs(data.date);
...
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <title>Nueva reserva - TuBarber</title>
</head>
<body style="margin:0;padding:0;background-color:#0D0D0D;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#0D0D0D;padding:40px 16px;">
    <tr>
            ...
            <td style="background-color:#1A1A1A;border:1px solid #C9A84C;border-radius:12px;padding:32px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                ...
                <tr>
                  <td align="center" style="padding-bottom:32px;">
                    <p style="margin:0;font-size:14px;color:#999999;">
                      Hola <strong style="color:#F5F5F0;">${data.barberName}</strong>, tienes un nuevo turno agendado.
                    </p>
                  </td>
                </tr>
                ...
                <tr>
                  <td style="padding-bottom:24px;">
                    ...
                    <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
                      <tr>
                        <td style="padding:14px 16px;background-color:rgba(255,255,255,0.04);border-radius:8px;border-left:3px solid #C9A84C;">
                          <p style="margin:0 0 3px;font-size:11px;color:#888888;text-transform:uppercase;letter-spacing:1px;">Cliente</p>
                          <p style="margin:0;font-size:16px;font-weight:700;color:#F5F5F0;">${data.clientName}</p>
                          ${data.clientPhone ? `<p style="margin:4px 0 0;font-size:13px;color:#888888;">${data.clientPhone}</p>` : ''}
                        </td>
                      </tr>
                    </table>
                ...
                <tr>
                  <td align="center">
                    <a href="${agendaUrl}" style="display:inline-block;padding:14px 36px;background-color:#C9A84C;color:#0D0D0D;font-size:14px;font-weight:800;text-decoration:none;border-radius:8px;letter-spacing:1px;text-transform:uppercase;">
                      Ver agenda
                    </a>
                  </td>
                </tr>
...
export async function sendBarberReservationAlert(data: BarberReservationAlertData): Promise<void> {
  if (!isEmailConfigured()) return;
  ...
  await resend.emails.send({
    from: fromAddress,
    to: data.barberEmail,
    subject: `✂ Nueva reserva: ${data.clientName} — ${data.time} hs`,
    html: buildBarberAlertHtml(data),
  });
}
